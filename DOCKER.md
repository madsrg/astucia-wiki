...
One container running four processes under supervisord:
...
| **mercure** | The realtime hub. nginx reverse-proxies `/.well-known/mercure` to it. Not started when `ENABLE_REALTIME=false` |

Base image `php:8.5-fpm-alpine`, about **250 MB**. A wiki is an appliance: one image
...
| `AGENT_JOB_RUNNER_INTERVAL_MINUTES` | `2` | Cron interval for AI jobs; also the ETA users are shown |
| `AGENT_JOB_RUNNER_SLOTS` | `2` | How many job runs may overlap. Each slot is another simultaneous LLM call |
...
The UI holds one long-lived SSE connection. On HTTP/1.1 a browser allows only ~6 connections
per origin and each tab's stream takes one, so a few tabs exhaust the budget and ordinary
requests start queueing behind them. Over HTTP/2 the streams are multiplexed onto one
connection and it is a non-issue.

It works over plain HTTP — it is just the case where a handful of tabs will bite. If you
terminate TLS at a reverse proxy (the normal deployment), enable HTTP/2 there and there is
nothing to think about. If you cannot, set `ENABLE_REALTIME=false` and the wiki polls exactly
as every release before this one did.

The bundled hub is the [Mercure](https://mercure.rocks) reference implementation, **AGPL-3.0**
and shipped unmodified; its licence and copyright notice are in the image at
`/usr/share/licenses/mercure/`. The wiki itself is unaffected by that licence — upstream is
explicit that it "applies only to the hub server itself, not to software using this hub" —
and the hub runs as a separate process.
...
```
ENABLE_CRON=true
AGENT_JOB_RUNNER_INTERVAL_MINUTES=2     # -> */2 * * * *
...
*/2 * * * * docker exec -u www-data astucia-wiki php /var/www/html/run_ai_agent_jobs.php
